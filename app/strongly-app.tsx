...
export default function StronglyApp({ identity }: { identity: Identity }) {
  ...
  function toggleQuest(group: "required" | "bonus" | "weekly", id: string) {
  ...
  function buy(id: string, price: number) {
    if (owned.includes(id)) { setTheme(id); setToast("Cosmetic equipped"); }
    else if (coins >= price) { setCoins(coins - price); setOwned([...owned, id]); setTheme(id); setToast("New cosmetic unlocked"); }
  ...
  const content = useMemo(() => {
    ...
    return <Settings identity={identity} />;
...
function Goals() {
  ...
  const labels = ["Choose a training plan", "Run 5K without stopping", "Complete a 10K", "Finish a 10-mile run", "Race day"];
  return <section className="page-section"><PageHeader eyebrow="LONG-TERM GOALS" title="Build your legend" copy="Break ambitious goals into milestones you can conquer." button="+ New goal" />
    <div className="goals-grid">
      <article className="panel featured-goal"><div className="goal-rune">◆</div><div className="goal-meta"><span>ACTIVE · FITNESS</span><small>Target: Oct 18, 2026</small></div><h2>Run my first half marathon</h2><p>Build endurance, stay consistent, and cross the finish line strong.</p><div className="goal-progress large"><span style={{ width: `${(done.filter(Boolean).length / 5) * 100}%` }} /></div><b>{done.filter(Boolean).length} of 5 milestones complete</b><div className="milestones">{labels.map((label, i) => <button className={done[i] ? "done" : ""} key={label} onClick={() => setDone(done.map((v, x) => x === i ? !v : v))}><i>{done[i] ? "✓" : i + 1}</i><span>{label}<small>{done[i] ? "Completed" : i === 3 ? "Linked to this week’s quest" : "+150 coins"}</small></span><em>{done[i] ? "DONE" : "+150 ◈"}</em></button>)}</div></article>
...
function Settings({ identity }: { identity: Identity }) {
  return <section className="page-section"><PageHeader eyebrow="PROFILE & SECURITY" title="Your adventurer’s record" copy="Manage your identity, timezone, and account access." />
    <div className="settings-grid"><section className="panel settings-panel"><h3>Profile</h3><label>Display name<input defaultValue={identity.fullName ?? "Hero"} /></label><label>Email address<input defaultValue={identity.email} disabled /></label><label>Timezone<select defaultValue="America/New_York"><option>America/New_York</option><option>America/Chicago</option><option>America/Denver</option><option>America/Los_Angeles</option><option>Europe/London</option></select></label><button className="button button-gold">Save changes</button></section><aside><section className="panel security-card"><span>✦</span><div><h3>Passwordless security</h3><p>Your account is protected by managed sign-in. No password is stored by STRONGLY.</p><a href="/signout-with-chatgpt?return_to=/">Sign out</a></div></section><section className="panel settings-panel"><h3>Week preferences</h3><label className="toggle-row"><span><b>Week starts Sunday</b><small>Your campaigns close Saturday at midnight.</small></span><input type="checkbox" defaultChecked /></label><label className="toggle-row"><span><b>Reduced motion</b><small>Minimize completion animations.</small></span><input type="checkbox" /></label></section></aside></div>
  </section>;
}
